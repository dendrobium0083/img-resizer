...
| バージョン | 1.0.0 |
| 作成日 | 2024年 |
| 最終更新日 | 2025年12月2日（ImageSharp導入完了） |
| 作成者 | - |
| 承認者 | - |
...
- **画像処理**: SixLabors.ImageSharp（クロスプラットフォーム対応）
...
設定クラスは`IOptions<ImageResizeSettings>`パターンで使用されます。

### 6.3 設定値の検証

アプリケーション起動時に設定値を検証する機能を実装しています。

**実装クラス**: `ImageResizeSettingsValidator`（`IValidateOptions<ImageResizeSettings>`を実装）

**検証タイミング**: アプリケーション起動時（`ValidateOnStart()`）

**検証内容**:
- 必須項目のチェック（OutputDirectory、TargetSize、AllowedExtensions）
- 値の範囲チェック（Width/Height、MaxFileSize、PaddingColor、ImageQuality）
- 拡張子の形式チェック（`.`で始まること）

無効な設定がある場合、アプリケーションは起動時にエラーを発生させ、起動を停止します。
