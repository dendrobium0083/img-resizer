...
アプリケーション起動時に設定値を検証します。無効な設定がある場合、起動を停止します。

**実装クラス**: `ImageResizeSettingsValidator`（`IValidateOptions<ImageResizeSettings>`を実装）

1. **必須項目のチェック**
   - OutputDirectory: 必須（空文字列不可）
   - TargetSize: 必須（null不可）
   - AllowedExtensions: 必須（1つ以上必要）

2. **値の範囲チェック**
   - TargetSize.Width/Height: 正の値（1以上）
   - MaxFileSize: 正の値
   - PaddingColor.R/G/B/A: 0～255（オプション）
   - ImageQuality.JpegQuality: 1～100（オプション）
   - ImageQuality.PngCompressionLevel: 0～9（オプション）

3. **形式チェック**
   - AllowedExtensions: 各拡張子が`.`で始まること

#### 3.7.5 実装レイヤー
- **Infrastructure層**: `ImageResizeSettings`クラス、`ImageResizeSettingsValidator`クラス
- **Presentation層**: `Program.cs`で設定を読み込み、検証を登録（`ValidateOnStart()`）
...
#### 3.8.1 機能概要
アプリケーションの動作を記録するロギング機能。構造化ログを実現するため**Serilog**を使用しています。
...
HTTPリクエストの自動ログ記録機能を実装しています。

- **ログレベル**: エラー時はError、5秒以上はWarning、それ以外はInformation
- **追加情報**: RequestHost、RequestScheme、RemoteIP、UserAgent

#### 3.8.6 実装レイヤー
- **全レイヤー**: `ILogger<T>`を使用（Serilogと互換性あり）
- **Presentation層**: `Program.cs`でSerilogを設定、リクエストログを有効化
