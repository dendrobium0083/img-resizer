...
#### 2.1.1 Result クラス（Resultパターン）

**名前空間**: `ImgResizer.Domain.Common`

**実装日**: 2025年12月1日

**概要**:
Resultパターンを実装する基底クラス。正常系と異常系を明示的に表現し、例外を使用せずにエラー情報を返却します。
...
// 失敗（値なし）
return Result.Failure("ERROR_CODE", "エラーメッセージ");

// 失敗（値あり）
return Result.Failure<byte[]>("ERROR_CODE", "エラーメッセージ");
```

**設計上の考慮事項**:
1. **不変性**: すべてのプロパティはgetterのみで、インスタンス作成後は変更不可
2. **バリデーション**: コンストラクタで成功時/失敗時の状態を検証
3. **型安全性**: ジェネリクスを使用して型安全な値の保持
4. **明示的な状態**: IsSuccess/IsFailureで状態を明確に表現

#### 2.1.2 ImageFile エンティティ
...
#### 2.1.3 IImageRepository インターフェース

**名前空間**: `ImgResizer.Domain.Interfaces`

**最終更新**: 2025年12月1日（Resultパターン対応）
...
| メソッド名 | 戻り値 | 説明 |
|-----------|--------|------|
| ReadImageAsync | `Task<Result<byte[]>>` | 画像ファイルを非同期で読み込む（Resultパターン） |
| SaveImageAsync | `Task<Result>` | 画像データを非同期で保存する（Resultパターン） |
| FileExists | `bool` | ファイルの存在を確認する |
| GetOutputPath | `string` | 出力ファイルパスを生成する |

**変更履歴**:
- 2025-12-01: Resultパターンに対応、ReadImageAsyncとSaveImageAsyncの戻り値をResultに変更
- GetOutputPathにresizeModeパラメータを追加

#### 2.1.4 IImageResizeService インターフェース
...
| メソッド名 | 戻り値 | 説明 |
|-----------|--------|------|
| ResizeToSquareAsync | `Task<Result<byte[]>>` | 画像を正方形にリサイズする（非同期、Resultパターン） |
...
**変更履歴**:
- 2025-12-01: Resultパターンに対応、ResizeToSquareAsyncの戻り値をResult<byte[]>に変更
- extensionパラメータを追加して画像形式に応じた品質設定を可能に
...
**責務**:
- 画像リサイズ処理の実装
- SixLabors.ImageSharpライブラリの使用（クロスプラットフォーム対応）
...
#### 2.3.4 ImageResizeSettingsValidator クラス
...
2. **クロップサイズの決定**
   ...
   {
       // 拡大処理（クロップなし）
       using var resizedImage = originalImage.CloneAs<Rgba32>();
...
4. **中央部分の切り出しとリサイズ**
   ```csharp
   using var croppedImage = originalImage.CloneAs<Rgba32>();
...
5. **画像データの変換**
   ```csharp
   using var memoryStream = new MemoryStream();
   croppedImage.Save(memoryStream, GetImageEncoder(extension));
   return memoryStream.ToArray();
   ```
...
**ログ出力先**:
- **コンソール**: 開発時の確認用
- **ファイル**: `logs/img-resizer-{日付}.log`（全ログ、30日保持、100MB制限）
...
|-----------|------|----------|--------|
| 1.0.0 | - | 初版作成 | - |
| 1.1.0 | 2025-12-01 | Resultパターンの追加、IImageRepository/IImageResizeServiceの更新 | AI Assistant |
